...
def f_error(N, out, pred):
    somme = 0
    for i in range(N):
        somme += (out[i] - pred[i])**2
    return (1/(2*N)) * somme
...
def main():
    ...
    while error > 0.01:
        ...
        # Calcul de l'erreur
        outL1 = forwardPropagationLayer(points, weightsLayer1, biasLayer1)
        outL2 = forwardPropagationLayer(outL1, weightsLayer2, biasLayer2)
        error = f_error(4, outL2, labels)
        noEpochs = noEpochs + 100
    ...
    print("Error = " + str(error) + " & noEpochs = " + str(noEpochs))
